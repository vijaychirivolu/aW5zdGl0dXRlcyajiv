...
class InstitutesController extends AppController {
    ...
    public $uses = array('Institute', 'UploadHistory', 'InstituteSetting','InstituteTiming','Skill');
    ...
    function beforeFilter() {
        ...
        $this->superadminActions = array('admin_index', 'admin_setup', 'admin_uploadInstituteData','admin_timings','admin_settings','admin_skills');
    ...
    public function admin_index() {
        //$this->Session->write('Auth.User.user_role', 1001);
        ...
        if ($this->RequestHandler->responseType() == 'json') {
            ...
            if (isset($result["aaData"])) {
                foreach ($result["aaData"] as $key => $val):
                    $formattedArray[$key]["Institute"]["name"] = '<a href="' . Router::url('/', true) . '/dashboards/account/' . $val["Institute"]["id"] . '">' . implode('-', array_map('ucwords', explode('-', strtolower(addslashes($val["Institute"]["name"]))))) . '</a>';
                    ...
                    $formattedArray[$key]["Institute"]["id"] = '<div class="btn-group"><a class="btn-white btn btn-xs" href="' . Router::url('/', true) . 'admin/institutes/timings/' . $val["Institute"]["id"] . '">' . __("Timings") . '</a><a class="btn-white btn btn-xs" href="' . Router::url('/', true) . 'admin/institutes/settings/' . $val["Institute"]["id"] . '">' . __("Settings") . '</a><a class="btn-white btn btn-xs" href="' . Router::url('/', true) . 'admin/institutes/skills/' . $val["Institute"]["id"] . '">' . __("Skills") . '</a><a class="btn-white btn btn-xs" href="' . Router::url('/', true) . 'admin/institutes/setup/' . $val["Institute"]["id"] . '">' . __("Edit") . '</a><a class="btn-white btn btn-xs delete-confirm-alert" href="' . Router::url('/', true) . 'admin/states/delete/' . $val["Institute"]["id"] . '" data-message = "You are permenantly deleting this institute!">' . __("Delete") . '</a></div>';
                endforeach;
            }
     ...
     *    or MissingViewException in debug mode.
     */
    public function admin_timings($instituteId) {
        if ($instituteId != "") {
            ...
            if (!empty($data)) {
                ...
                $updateConditions = array(
                  'InstituteTiming.row_status' => 1,
                  'InstituteTiming.institute_id' => $instituteId  
                );
                $this->InstituteTiming->updateInstituteTimingDetails($updateData,$updateConditions);
                if ($this->InstituteTiming->saveMany($data)) {
                    $msg = __('Institute timings are saved successfully.');
                    $this->_setFlashMsgs($msg, 'success');
                    $this->redirect(array('controller' => 'institutes', 'action' => 'admin_index'));
                } else {
                    $msg = __('Saving failed due to below errors!');
                    $this->_setFlashMsgs($msg, 'danger');
                }
            ...
            $timings = array('00:00'=>'00:00','00:30' => '00:30','01:00' => '01:00','01:30' => '01:30','02:00' =>'02:00','02:30' =>'02:30','03:00' => '03:00','03:30' => '03:30','04:00' => '04:00','04:30' => '04:30','05:00' => '05:00','05:30' => '05:30','06:00' => '06:00','06:30' => '06:30','07:00' => '07:00','07:30'=>'07:30','08:00' => '08:00','08:30' =>'08:30','09:00' =>'09:00','09:30' =>'09:30','10:00' =>'10:00','10:30' =>'10:30','11:00' =>'11:00','11:30' =>'11:30','12:00' =>'12:00','12:30' =>'12:30','13:00' =>'13:00','13:30' =>'13:30','14:00' =>'14:00','14:30' =>'14:30','15:00' =>'15:00','15:30' =>'15:30','16:00' =>'16:00','16:30' =>'16:30','17:00' =>'17:00','17:30' =>'17:30','18:00' =>'18:00','18:30' =>'18:30','19:00' =>'19:00','19:30' =>'19:30','20:00' =>'20:00','20:30' =>'20:30','21:00' =>'21:00','21:30' =>'21:30','22:00' =>'22:00','22:30' =>'22:30','23:00' =>'23:00','23:30' =>'23:30');
            $instituteHours = array();
            $result = $this->InstituteTiming->fetchTimingsDetailsByInstituteId($instituteId);
            foreach ($weekData as $k => $r):
                $instituteHours[$k] = $r;
                foreach ($result as $val):
                    if ($r["GroupValue"]["id"] == $val["InstituteTiming"]["week_id"]) {
                ...
                endforeach;
            endforeach;
            $this->set(compact('instituteHours','timings',"instituteId"));
        } else {
            $this->redirect($this->UserAuth->redirect());
        }
    }
     ...
     *    or MissingViewException in debug mode.
     */
    public function admin_settings($instituteId) {
        if ($instituteId != "") {
            $this->Session->write('Auth.User.institute_id', $instituteId);
            if (!empty($this->request->data)) {
                $data = $this->request->data;
            ...
            $result = $this->InstituteSetting->fetchSettingDetailsByInstituteId($instituteId);
            $this->request->data = $result;
            $breakTimes = array(5 => '5 minutes', 10 => '10 minutes', 15 => '15 minutes');
            $lunchTimes = array(30 => '30 minutes', 45 => '45 minutes', 60 => 'Hourly');
            $periodTimes = array(30 => '30 minutes', 45 => '45 minutes', 60 => 'Hourly');
            $maxAllowedPeriods = array(4 => '4 Periods', 5 => '5 Periods', 6 => '6 Periods', 7 => '7 Periods', 8 => '8 Periods');
            $this->set(compact("breakTimes", "lunchTimes", "periodTimes", "maxAllowedPeriods","instituteId"));
        } else {
            $this->redirect($this->UserAuth->redirect());
        }
    }
     ...
     *    or MissingViewException in debug mode.
     */
    public function admin_skills($instituteId) {
        if ($instituteId != "") {
            $this->Session->write('Auth.User.institute_id', $instituteId);
            $data = $this->request->data;
            if (!empty($data)) {
                ...
                $this->Skill->updateSkillDetails($updateData,$updateConditions);
                //pr($data);exit;
                if ($this->Skill->saveMany($data)) {
                    $msg = __('Institute Skills are saved successfully.');
                    $this->_setFlashMsgs($msg, 'success');
                    $this->redirect(array('controller' => 'institutes', 'action' => 'admin_index'));
                } else {
                    $msg = __('Saving failed due to below errors!');
            ...
            $defaultSkills = array();
            $defaultSkills = $this->Skill->fetchSkillsDetailsByInstituteId($instituteId);
            if (empty($defaultSkills)) {
                $groupResult = $this->Custom->fetchGroupValuesById(10000);
                if (!empty($groupResult)) {
                    foreach($groupResult as $key=>$res):
                        $defaultSkills[$key]["Skill"]["id"] = $res["GroupValue"]["id"];
                        $defaultSkills[$key]["Skill"]["name"] = $res["GroupValue"]["name"];
                    endforeach;
                }
            } else {
                foreach($defaultSkills as $key=>$res):
                    $defaultSkills[$key]["Skill"]["checked_values"] = $res["Skill"]["name"];
                endforeach;
            }
            $this->set(compact('defaultSkills',"instituteId"));
        } else {
            $this->redirect($this->UserAuth->redirect());
        }
    }
