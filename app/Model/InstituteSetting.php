 ...
 *
 * @category    InstituteSettingModel
 * @package     Models
 ...
 * @package     Models
 * @author      Vijay.Ch <vijay.ch@vendus.com>
 * @fileName    InstituteSettingModel.php
 * @description Used for user queies and validations
 * @license     http://www.opensource.org/licenses/mit-license.php MIT License
 * @link        http://localhost/cacsv2/index
 */
class InstituteSetting extends AppModel {
    
    public $name = 'InstituteSetting';
    /**
     ...
     *    or MissingViewException in debug mode.
     */
    public function updateInstituteSettingDetails($updateData, $conditions) {
        try {
     ...
     *    or MissingViewException in debug mode.
     */
    public function fetchSettingDetailsByInstituteId($instituteId) {
        try {
            $result = $this->find("first", array(
                'conditions' => array(
                'InstituteSetting.row_status' => 1,
                'InstituteSetting.institute_id' => $instituteId
            )
            ));
